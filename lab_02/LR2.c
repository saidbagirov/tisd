...
int main(void)
{
	...
	printf("её автоматически. Далее вы сможете удалять записи из таблицы, добавлять их,\n");
	printf("сортировать спектакли по самым дешёвым билетам с помощью сортировок по ключам и самой таблицы и\n");
	...
	action = -1;
	while (action) // Основное меню действий
	{
		printf("\nВыберите действие:\n");
		printf("1 - вывести таблицу\n");
		printf("2 - добавить запись в конец таблицы\n");
		printf("3 - удалить запись по её индексу\n");
		printf("4 - отсортировать ключи по меньшей цене билетов и вывести их\n");
		printf("5 - отсортировать таблицу по меньшей цене билетов и вывести её\n");
		...
		if (read_int(&action))
		{
			if (action == 1 || action == 2 || action == 3 || action == 4 || action == 5 || action == 6 || action == 7 || action == 8 || action == 9 || !action)
			{
				...
				else if (action == 3) // Удалить запись по еёиндексу
				{
					if (count)
				...
				else if (action == 5) // Отсортировать таблицу и вывести её на экран
				{
					if (count)
					{
