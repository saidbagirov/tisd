...
void add_struct(struct spectacle *table, int i)
{
	...
	count_let = 0;
	while (!count_let || count_let > THEATER) // Проверка на правильность ввода строкового типа
	{
		printf("\nВведите название театра (до 35 символов)\n");
		count_let = read_line(table[i].theater, THEATER);
		if (!count_let || count_let >= THEATER)
		{
			count_let = 0;
	...
	while (!count_let || count_let > NAME)
	{
		printf("\nВведите режиссёра (до 40 символов)\n");
	...
	else if (action == 3) // Музыкальный
	{
		...
		count_let = 0;
		while (!count_let || count_let > THEATER)
		{
...
// Функция удаления записи по её индексу
void remove_struct(struct spectacle *table, int *count)
{
	...
	index--;
	for (int i = index; i < *count - 1; i++) // Сдвиг массива структур на 1 начиная с введённого индекса
		table[i] = table[i + 1];
	(*count)--;
}