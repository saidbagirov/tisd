...
void classic_sum(const matrix_t matrix_a, const matrix_t matrix_b, matrix_t *const matrix_res, uint64_t *const ticks)
{
	...
	for (int row = 0; row < matrix_a.rows; ++row) 
		for (int col = 0; col < matrix_a.columns; ++col)
			*(*(matrix_res->matrix + row) + col) = *(*(matrix_a.matrix + row) + col) + *(*(matrix_b.matrix + row) + col);
	uint64_t end = tick();
	*ticks = end - start;
}
/*
...
{
	int doom_seed[] = {151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30,
