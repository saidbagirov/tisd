		...
		switch (choice)
		{
			...
			case 1:
				...
				if (!temp)
				{
					...
					printf("Кажется, стек переполнился... (Количество добавленных элементов: %d)\n", amount);
					break;
				}
				times_array[LIST_STACK_PUSH].amount = amount;
				node_t *tmp = temp;
			...
			case 3:
				...
				if ((err_code = cleann_l(&ps, amount, &times_array[LIST_STACK_POP].time, free_zones)) != amount)
				{
					times_array[LIST_STACK_POP].amount = err_code;
					printf("Кажется, стек закончился... (Количество очищенных элементов: %d)\n", err_code);
					break;
				}
